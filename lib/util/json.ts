export function jsonHTTPResponse(
  ...
  const jsonhttp: JSONHTTPResponse = {
    content: {
      status,
      message,
      context,
    ...
    alt: {
      status,
      statusText: lookup[`${status}`]?.short ?? 'Unexpected Status Code',
    },
  };
  return Response.json(jsonhttp.content, jsonhttp.alt);
}
...
const lookup: {
  [key: string]: { short: string; large: string };
} = {
  '100': {
    'short': 'Continue',
    'large':
